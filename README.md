...
- SolanaのTxを判別しCryptactのカスタムファイルを自動生成するPythonツール  
- CryptactはSolanaに対応しているが使い物にならなかったので自作（おまけでSuiにも対応）  
- 対応取引はスワップ、ブリッジ、一部のレンディング、送金など  
- NFT・流動性提供には未対応（未対応取引は「不明」と区別して出力）  

## 注意事項
- **自己責任での使用をお願いします**  
- Cryptactのヘルプファイルに則り分類・転記していますが、間違いがある可能性があります  
- 個人用のため対応取引に偏りがあります  
...
- ガス代がマイナスならBONUS  
通常のガス代（Suiが減る）：DEFIFEE,SENDFEE  
ガス代がマイナス（Suiが増える）：BONUS  
...
このソフトウェアの著作権表示（「Copyright © 2025 Salineras」）と、このライセンスの全文（または英語の原文）を、ソースコードや、ソースコードに同梱したライセンス表示用の別ファイルなどに掲載してください。  
